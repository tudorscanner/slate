# Hotels Live Prices (new)

<aside class="warning">
  Please note that the first Skyscanner Hotels API will be depricated in December 2017. Legacy documentation is available <a href="https://support.business.skyscanner.net/hc/en-us/articles/212098705-Hotels-Price-List-and-Hotels-Details-service" target="_blank">here</a> but please insure you migrate your application to the new API before that date. 

  All the documentation below is for the new Hotels API.
</aside>

Retrieve live prices for hotels in any given [location](#car-hire-and-hotels). We get query all our partner providers to get an extensive list of hotels and hotel prices.

<aside class="notice">
It may take up to a minute to update all the results. You should keep polling the session at a suitable interval (e.g. every 1-3 seconds) whilst there are still updates pending.
</aside>

## First call

```shell
curl "https://gateway.skyscanner.net/hbe-bellboy/v1/web/prices/search/entity/27548283
  ?market=ES&locale=es-ES
  &checkin_date=2017-12-11&checkout_date=2017-12-13
  &currency=EUR&adults=2&rooms=1&apikey={apikey}"
  -H "x-user-agent: M;B2B" 
```
...
`GET https://gateway.skyscanner.net/hbe-bellboy/v1/web/prices/search/entity/{entityId}
  ?market={market}&locale={locale}
  &checkin_date={checkinDate}&checkout_date={checkoutDate}
  &currency={currency}&adults={adults}&rooms={rooms}`

*TRY IT OUT*

[![Run in Postman](https://run.pstmn.io/button.svg)](https://www.getpostman.com/collections/3f6701b598a057c8bd58)
